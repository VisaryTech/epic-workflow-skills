---
name: epic-reviewer
description: "Проверка готовности эпика к переходу из `to-approve` в `approved` или `to-define` через структурированное review по блокам требований, критическим пробелам, рискам и противоречиям. Использовать, когда нужно: проверить эпик, перепроверить epic/recheck, оценить полноту требований, принять решение APPROVED или TO DEFINE, выявить gaps/conflicts/risks, заново прочитать эпик из ERP и вернуть вердикт с обновлением меток и комментариев."
---
...
- Compact template должен быть ориентирован на автора эпика: verdict, краткая причина, блокирующие замечания, проверяемые действия, интеграционные риски, похожие эпики и ERP sync.
- В compact template не выводи внутренние категории проверки, методологические классификации требований и сырые enum-статусы.
- В compact template технические статусы переводи в человекочитаемый русский текст, например: `missing` → `отсутствует`, `not_checked` → `не проверялась`, `related_only` → `похожий эпик, scope отличается`, `add_links` → `добавить ссылку/контекст`.
- Счётчики `Критические блокеры` и `Разделов с замечаниями` в compact template не выводи; вместо них используй строку `Причина: <N> блокирующих замечаний`.
- `Ключевые замечания` в compact template не используй; блокирующие проблемы выводи в секции `Блокирует APPROVED`.
- `Что нужно закрыть` в compact template оформляй как checklist с проверяемыми условиями.
- Full template в чат допустим только если это явно полезно и пользователь не просил скрыть служебные поля.
- Если во входе есть `epicUrl`, поле `Эпик: <epic_url>` заполняй этим значением.
- Если во входе есть только `epicId` и ссылку корректно определить нельзя, поле `Эпик:` можно опустить.
