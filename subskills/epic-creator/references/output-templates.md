...
## Template A0 — initial intake missing

Использовать, когда пользователь просит создать epic, но не дал minimum context или дал только намерение.
Не выводить отдельный список недостающих полей перед шаблоном.
Заполняемый шаблон всегда отдавать fenced-блоком `text`.
...
- Если отсутствует почти весь minimum context, используй `Template A0`: не дублируй поля списком, а дай один copy-ready `text`-шаблон.
